...
def interfaceResidues(cmpx, cA='c. A', cB='c. B', cutoff=1.0, selName="interface"):
    ...
    # PYMOL计算面积(get_area)时,依赖的dot_solvent参数
	...
	# chA, chB = "chA", "chB"
	chain_id_A = cA.split('.')[-1].strip()  # 从 'c. A' 中提取 'A'
	chain_id_B = cB.split('.')[-1].strip()  # 从 'c. B' 中提取 'B'
	chA = f"chain_{chain_id_A}"
	chB = f"chain_{chain_id_B}"
